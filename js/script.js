...
/***/ (function(module, exports) {
  ...
  window.addEventListener("DOMContentLoaded", function() {
    ...
    const getResource = async (url) => {
      ...
      if(!res.ok) {
        throw new Error(`Couldn't fetch ${url}, status: ${res.status}`); //оператор throw отвечает за выброс ошибки, которую мы создаем
      }
    ...
    function getDynamicInformation(selector) {
      ...
      input.addEventListener("input", () => {
        ...
        if (input.value.match(/\D/g)) { // проверка для инпутов, если не число - то рамка красная
